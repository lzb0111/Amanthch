        ...
        <div className="space-y-6 text-gray-700 dark:text-gray-300 leading-relaxed">
          ...
          <div className="p-6 bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-600">
            ...
            <p className="text-gray-700 dark:text-gray-300">
              {isZh
                ? '手动计算仓位容易出错，尤其是交叉货币对（如GBP/JPY、EUR/AUD）每点价值不固定。建议使用专业仓位计算器（如GannWheel Position Calculator），只需输入账户、风险%、止损点数，即可自动计算最佳手数，避免计算错误导致的风险失控。'
                : 'Manual position calculation is error-prone, especially for cross pairs (GBP/JPY, EUR/AUD) with variable pip values. Recommend using professional position calculator (like GannWheel Position Calculator), just input account, risk %, stop pips to automatically calculate optimal lot size, avoiding calculation errors causing risk loss of control.'}
            </p>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 bg-red-50 dark:bg-red-900/20 border-2 border-red-600">
            ...
            <p className="text-green-600 dark:text-green-400 font-bold">
              ✅ {isZh ? '正确做法：' : 'Solution:'}{' '}
              {isZh
                ? '了解货币对相关性，避免同时交易高度相关的货币对。如果必须交易，将总风险控制在2-3%内（如每对1%）。建议：同时最多持有2-3个仓位，且选择低相关性货币对（如EUR/USD + GBP/JPY + AUD/NZD）。使用GannWheel的相关性工具识别风险。'
                : 'Understand currency pair correlation, avoid simultaneously trading highly correlated pairs. If must trade, control total risk within 2-3% (e.g., 1% each). Recommend: hold maximum 2-3 positions simultaneously, choose low-correlation pairs (e.g., EUR/USD + GBP/JPY + AUD/NZD). Use GannWheel correlation tool to identify risks.'}
            </p>
          </div>
        ...
        <div className="space-y-6 text-gray-700 dark:text-gray-300 leading-relaxed">
          ...
          <div className="p-6 bg-black dark:bg-white text-white dark:text-black">
            ...
            <p className="mt-3 text-sm">
              {isZh
                ? 'GannWheel 提供完整的风险管理工具套件，帮助你实现自动化、系统化的风险控制。'
                : 'GannWheel provides complete risk management tool suite to help you achieve automated, systematic risk control.'}
            </p>
          </div>
